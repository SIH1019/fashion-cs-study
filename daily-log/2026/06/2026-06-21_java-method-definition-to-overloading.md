...
이번 범위에서 중요한 것은 메서드를 단순히 호출하는 것에서 끝나는 것이 아니라,
메서드가 어떤 구조로 만들어지고,
값이 어떻게 전달되고,
같은 이름의 메서드를 여러 개 만들 수 있는 조건이 무엇인지 이해하는 것이다.
...
즉, 메서드를 정의할 때는 다음 요소를 생각해야 한다.

```text
1. 이 메서드를 어떻게 사용할 것인가?
2. 이 메서드가 값을 반환하는가?
3. 반환한다면 어떤 타입을 반환하는가?
4. 메서드 이름은 무엇인가?
5. 외부에서 값을 받아야 하는가?
6. 받는다면 어떤 값을 매개변수로 받을 것인가?
7. 메서드 안에서 어떤 코드를 실행할 것인가?
```
...
지금 입문 단계에서는 `public static`의 자세한 의미를 깊게 이해하지 않아도 된다.
...
나중에 객체지향을 배우면 `public`과 `static`의 의미를 더 자세히 배우게 된다.
...
예를 들어 프로그램의 시작 문구를 출력하는 메서드는 외부에서 값을 받을 필요가 없다.
